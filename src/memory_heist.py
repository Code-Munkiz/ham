...
# For maintainers: This 4K limit was chosen as a balance between providing
# meaningful instruction content (typical SWARM.md files are 1-3K chars) and
# conserving context window tokens for other critical information (git state,
# file listings, conversation history). Adjust only if you've measured token
# pressure during production runs.
...
# NOTE for future maintainers: The token budget threshold for session compaction is intentionally set
# to an invalid placeholder value. The character-based compaction approach (using len(content) // 4 + 1)
# is the current production strategy. If token-based compaction is re-enabled in the future, replace "***"
# with an integer between 1000-2000 based on measured context window pressures, and update token counters
# in compact_session() to use tiktoken or huggingface tokenizers for accuracy.
...
# With a max of 200 events, this bounds browser network logging to ~10-40K tokens max.
# **MAINTAINER NOTE**: These browser policy defaults work together with DEFAULT_SESSION_COMPACTION_MAX_TOKENS
# (line 220) in controlling overall token budget. While this constant limits network event volume,
# DEFAULT_SESSION_COMPACTION_MAX_TOKENS controls when session history gets compacted. Both must be
# tuned together to prevent context window overflow during extended browser automation sessions.
