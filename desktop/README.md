...
- **Local Control Phase 2–3B:** narrow IPC (`ham-desktop:local-control-*`) + **`window.hamDesktop.localControl`** for status, policy/audit/kill-switch reads, sidecar **`getSidecarStatus`**, **`pingSidecarHealth`**, **`stopSidecar`**, **`startSidecar`** (start **blocked** while kill switch engaged — default), and **engage kill switch** only; default **`policy.json`** + redacted audit JSONL under userData; **disabled by default** — [`docs/desktop/local_control_v1.md`](../docs/desktop/local_control_v1.md), sidecar protocol [`docs/desktop/local_control_sidecar_protocol_v1.md`](../docs/desktop/local_control_sidecar_protocol_v1.md). **3B** = inert stdio child only (no tools/automation).
...
- Additional IPC: `window.__HAM_DESKTOP_BUNDLE__` and **`window.hamDesktop`** share the same `localControl` bridge (`getStatus`, `getPolicyStatus`, `getAuditStatus`, `getKillSwitchStatus`, `getSidecarStatus`, `pingSidecarHealth`, `stopSidecar`, `startSidecar`, `engageKillSwitch`) — see `preload.cjs`, `main.cjs`, `local_control_*.cjs`.
- **CLI (repo, no Electron):** `ham desktop local-control status|policy|audit|sidecar` (and `sidecar health|stop|start` = explicit **electron_only** stubs) and `ham desktop local-control kill-switch engage` (noop: directs to desktop); live sidecar lifecycle only in the packaged app.
