...
- **Local Control Phase 2–3A:** narrow IPC (`ham-desktop:local-control-*`) + **`window.hamDesktop.localControl`** for status, policy/audit/kill-switch reads, **`getSidecarStatus`** (mock), and **engage kill switch** only; default **`policy.json`** + redacted audit JSONL under userData; **disabled by default** — [`docs/desktop/local_control_v1.md`](../docs/desktop/local_control_v1.md), sidecar design [`docs/desktop/local_control_sidecar_protocol_v1.md`](../docs/desktop/local_control_sidecar_protocol_v1.md). **No** sidecar child process in 3A.
...
- Additional IPC: `window.__HAM_DESKTOP_BUNDLE__` and **`window.hamDesktop`** share the same `localControl` bridge (`getStatus`, `getPolicyStatus`, `getAuditStatus`, `getKillSwitchStatus`, `getSidecarStatus`, `engageKillSwitch`) — see `preload.cjs`, `main.cjs`, `local_control_*.cjs`.
- **CLI (repo, no Electron):** `ham desktop local-control status|policy|audit|sidecar` and `ham desktop local-control kill-switch engage` (noop: directs to desktop); live persistence only in the packaged app.
