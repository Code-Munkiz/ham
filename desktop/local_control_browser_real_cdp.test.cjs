...
test("realBrowserActionGates: kill switch blocks", () => {
  const g = realBrowserActionGates(
    baseRealPolicy({ kill_switch: { engaged: true, reason: "t" } }),
    "linux",
  );
  assert.equal(g.ok, false);
...
test("realBrowserActionGates: not armed blocks", () => {
  const g = realBrowserActionGates(
    baseRealPolicy({
      ...
      permissions: { real_browser_automation: false },
    }),
    "linux",
  );
  assert.equal(g.ok, false);
...
test("discoverChromiumExecutableWindows: finds Program Files chrome", () => {
  ...
  fs.accessSync = (p) => {
    ...
    if (s.includes("google\\chrome\\application\\chrome.exe")) return;
    throw new Error("missing");
  };
  try {
    ...
    assert.ok(
      found &&
        found
          .toLowerCase()
          .replace(/\//g, "\\")
          .includes("google\\chrome\\application\\chrome.exe"),
    );
  } finally {
    fs.accessSync = prevAccessSync;
    process.env.HAM_DESKTOP_CHROME_PATH = prevEnv.HAM_DESKTOP_CHROME_PATH;
      ...
      json: async () => [
        {
          type: "page",
          url: "about:blank",
          webSocketDebuggerUrl: "ws://localhost:9333/devtools/page/z",
        },
      ],
    };
  };
