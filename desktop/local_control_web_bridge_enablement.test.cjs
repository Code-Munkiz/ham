...
test("localWebBridgeDisabledReason: explicit disable is reported", () => {
  assert.equal(
    localWebBridgeDisabledReason({ envValue: "0", isPackaged: true }),
    "explicit_disabled",
  );
  assert.equal(
    localWebBridgeDisabledReason({ envValue: "off", isPackaged: false }),
    "explicit_disabled",
  );
});
...
test("localWebBridgeDisabledReason: dev default reason, packaged unset none", () => {
  assert.equal(
    localWebBridgeDisabledReason({ envValue: "", isPackaged: false }),
    "disabled_by_default_dev",
  );
