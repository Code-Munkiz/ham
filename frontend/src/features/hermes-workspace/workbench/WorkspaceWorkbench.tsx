...
function WorkbenchPreviewPanel({
  ...
  const cloudPreviewDisconnected = ["disabled", "experiment_not_enabled", "config_missing"].includes(
    cloudRuntimeState,
  );
  const primaryState =
    ...
    <div className="flex h-full min-h-0 flex-col gap-3 text-[12px] leading-relaxed text-white/70">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          variant="secondary"
          className="text-[11px]"
          data-testid="hww-preview-edit"
          disabled={!visualEditReady}
          title={
            visualEditReady
              ? "Opens visual edit flow (save requests under Advanced for now)"
              : "Available when preview is ready"
          }
        >
          Edit
        </Button>
      ...
      >
        {previewPhase === "ready" && previewUrl ? (
          ...
          <div
            ...
            data-testid={
              previewPhase === "no_project" ? "hww-preview-state-no-project" : "hww-preview-placeholder"
            }
          >
            <p
            ...
            {previewPhase === "error" ? (
              <p className="max-w-md text-[12px] text-amber-200/90" data-testid="hww-preview-state-error">
  ...
  return (
    <MutedPanel>
      ...
      <p className="text-white/55">
        Use <span className="font-medium text-white/65">Add project source</span> for a full ZIP when
