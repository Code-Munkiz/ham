...
export function WorkspaceCodingAgentsScreen({ workspaceId }: { workspaceId?: string }) {
  ...
  const refresh = React.useCallback(async () => {
    ...
    const [r, p, oc, settingsResult] = await Promise.all([
      fetchCursorReadiness(),
      ...
      fetchOpencodeReadinessForCodingAgentsScreen(),
      workspaceId ? fetchCodingAgentAccessSettings(workspaceId) : Promise.resolve(null),
    ]);
  ...
  return (
    <section className="flex flex-wrap items-center gap-3 rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-card)] px-4 py-3 shadow-[0_12px_40px_var(--theme-shadow)]">
      ...
      <button
        ...
        className="ml-auto flex items-center gap-1 rounded-md px-2 py-1 text-[11px] text-[var(--theme-muted)] hover:text-[var(--theme-text)] transition-colors"
...
// ---------------------------------------------------------------------------
// AgentSettingsPanel — normie-friendly builder settings (no provider internals)
// ---------------------------------------------------------------------------
