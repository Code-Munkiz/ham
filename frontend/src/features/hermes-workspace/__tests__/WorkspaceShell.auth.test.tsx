...
describe("WorkspaceShell auth gating", () => {
  ...
  it("does not prefetch chat sessions via workspaceSessionAdapter while signed out", async () => {
  ...
  it("does not prefetch chat sessions via workspaceSessionAdapter after workspace becomes ready", async () => {
    const list = vi.spyOn(workspaceSessionAdapter, "list").mockResolvedValue({
      sessions: [],
    });
