...
- **Deferred (unchanged direction):** no second orchestration harness, no FTS5 durable learning persistence yet, no Phase 4 Droid execution-safety hardening until executor-backed flows and mutating-command policy are ready for that pass.
...
**Blocked by**: No automatic `ProjectContext` rebuild after Droid subprocess
steps yet (executor exists; wiring refresh-after-mutation is follow-up).
