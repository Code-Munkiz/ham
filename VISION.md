...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 3B live governed controller scaffold:** Phase 2B remains execution-disconnected and Phase 3A `goham_controller.py` remains dry-run-only; `goham_live_controller.py` accepts prepared candidates, asks the governor, may call the guarded GoHAM bridge at most once for an original post, then stops. Live posting remains disabled by default; no scheduler, daemon, multi-action runner, xurl mutation, quote/reply/like/follow/DM, or live X+xAI acquisition wiring |
