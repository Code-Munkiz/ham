...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 4B Reactive Live Reply Canary:** Phase 2B remains execution-disconnected, Phase 3A `goham_controller.py` remains dry-run-only, and Phase 3B `goham_live_controller.py` remains original-post-only. Phase 4A `goham_reactive.py` still classifies prepared/read-only inbound mentions/comments into dry-run review/exception records. Phase 4B adds `reactive_reply_executor.py` and `goham_reactive_live.py` as a separate one-shot canary that can consume one prepared inbound or one Phase 4A `reply_candidate`, re-check policy/governor, send at most one reply through a narrow `POST /2/tweets` reply payload, journal as `execution_kind="goham_reactive_reply"`, then stop. Reactive budgets remain separate from broadcast caps; no scheduler, daemon, batch loop, original posts, quotes, DMs, likes/follows, xurl mutation, manual canary, broadcast executor, or Phase 2B execution |
