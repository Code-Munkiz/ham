...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 2A:** non-mutating social-agent scaffold + read/model smoke surfaces plus a separate manual canary executor for one `post` or `quote`; live execution remains disabled by default (`HAM_X_ENABLE_LIVE_EXECUTION=false`), is capped, requires `manual_confirm=true`, and is not called by scoring, autonomy, xAI drafting, read-only smoke, or queues |
