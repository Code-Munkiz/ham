...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **GoHAM v0 ops:** Phase 2B live-read/live-model dry-run remains execution-disconnected; guarded GoHAM bridge/policy supports a mocked/tested autonomous original-post path only when explicit GoHAM/live/autonomy gates, low-risk `auto_approve`, strict no-link/no-finance policy, caps, and idempotency pass; `goham_ops.py` adds read-only daily status/cap/journal/preflight helpers that do not import or call execution paths; live posting remains disabled by default |
