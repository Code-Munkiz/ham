...
| Managed Cloud Agent + mission record | `src/persistence/managed_mission.py`, `src/ham/managed_mission_wiring.py`, `src/api/cursor_settings.py`, `src/api/cursor_managed_*.py`, `src/ham/cursor_agent_workflow.py`, `src/ham/chat_operator.py`, Hermes Workspace chat (`/workspace/chat`) | Durable per-agent mission JSON + read APIs (list/detail, bounded **`/feed`**, follow-up **`/messages`**, **`/cancel`**); optional `project_id` on HAM launch for create-time `mission_deploy_approval_mode` snapshot; **Chat operator** can preview/launch Cursor Cloud Agent with `cursor_mission_handling: managed` — same managed prompt for digest/launch, `ManagedMission` row on successful API launch; Workspace chat **`?mission_id=`** deep links stay mission-scoped; dashboard chat uses structured `operator` via `/api/chat`; **not** a mission graph or Hermes-to-Cursor action loop; gap roadmap: `docs/ROADMAP_CLOUD_AGENT_MANAGED_MISSIONS.md` |
