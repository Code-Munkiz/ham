...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 4A dry-run Reactive Engine:** Phase 2B remains execution-disconnected, Phase 3A `goham_controller.py` remains dry-run-only, and Phase 3B `goham_live_controller.py` remains original-post-only. New `inbound_client.py`, `reactive_policy.py`, `reactive_governor.py`, and `goham_reactive.py` classify prepared/read-only inbound mentions/comments, apply separate reactive reply budgets/cooldowns/duplicate checks, and route reply candidates to review or unsafe items to exception records without provider writes. Reactive budgets are separate from broadcast caps; no scheduler, daemon, live replies, DMs, likes/follows, quote replies, xurl mutation, or Phase 2B execution |
