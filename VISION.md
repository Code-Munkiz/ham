...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 3A dry-run Firehose Controller:** Phase 2B remains execution-disconnected; guarded GoHAM bridge/daily runner still owns live original-post execution; new `goham_campaign.py`, `goham_governor.py`, and `goham_controller.py` add campaign profiles, deterministic firehose budgets/cooldowns/circuit breakers, and bounded dry-run candidate decisions with audit records only; live posting remains disabled by default |
