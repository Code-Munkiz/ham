...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 1E:** non-mutating social-agent scaffold with dry-run opportunity pipeline, decisioning-only autonomy engine, review/exception queues, redacted audit trace, xurl catalog linkage, safe smoke harness (`local`, `env`, `x-readonly`, `xai`, `e2e-dry-run`), and gated read-only `xurl search` smoke; `HAM_X_AUTONOMY_ENABLED=false`, `HAM_X_DRY_RUN=true`, `HAM_X_ENABLE_LIVE_SMOKE=false`, `mutation_attempted=false`, and `execution_allowed=false` preserve no-posting behavior |
