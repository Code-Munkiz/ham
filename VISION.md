...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 2C:** Phase 2B live-read/live-model dry-run remains execution-disconnected; new guarded GoHAM bridge/policy supports a mocked/tested autonomous original-post path only when explicit GoHAM/live/autonomy gates, low-risk `auto_approve`, strict no-link/no-finance policy, caps, and idempotency pass; live posting remains disabled by default |
