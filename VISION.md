...
| Managed Cloud Agent + mission record | `src/persistence/managed_mission.py`, `src/ham/managed_mission_wiring.py`, `src/api/cursor_settings.py`, `src/api/cursor_managed_*.py`, `src/ham/cursor_agent_workflow.py`, `src/ham/chat_operator.py`, Hermes Workspace (`WorkspaceManagedMissionsLivePanel`, `/workspace/chat`) | Durable per-agent mission JSON + read APIs (list/detail/feed with capped events); follow-up message and cancel routes append feed events and proxy Cursor when supported; `POST /api/cursor/agents/{id}/sync` returns persisted mission after observe; optional `project_id` on HAM launch for create-time `mission_deploy_approval_mode` snapshot; **Chat operator** can preview/launch with `cursor_mission_handling: managed` and mission-scoped chat via `?mission_id=`; **not** a mission graph or automatic Hermes-to-Cursor replan loop; gap roadmap: `docs/ROADMAP_CLOUD_AGENT_MANAGED_MISSIONS.md` |
