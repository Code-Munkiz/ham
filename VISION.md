...
| Workspace UI | `frontend/` (Vite + React), `desktop/` (Electron M1 shell) | Extracted workspace; TypeScript types aligned with persisted run / bridge shapes; optional **Clerk** (`VITE_CLERK_PUBLISHABLE_KEY`) for chat session JWT + `X-Ham-Operator-Authorization` for HAM confirm tokens; **desktop** is a thin Electron wrapper (`desktop/README.md`) with runtime `HAM_DESKTOP_API_BASE` / preload-injected config; **Settings → HAM + Hermes setup** offers allowlisted `hermes` **preset** runs (`runHermesPreset` IPC) plus **Local Control** **Phase 4A / 4B** (`window.hamDesktop.localControl`: status, policy v3 / audit / kill-switch, inert **sidecar** IPC, **Linux browser paths** — **Phase 4A:** `BrowserWindow` in main + `local-control-browser-*` IPC; **Phase 4B:** managed Chromium/Chrome with **127.0.0.1-only CDP** + `local-control-browser-real-*` IPC (navigate, **reload** Local Computer Control Phase 1A, screenshot, stop, **GoHAM v1 Slice 1–3** compact observe / bounded wait–scroll / enumerated candidate click / **Slice 2** rules-first **research loop** with optional schema-gated LLM planner interface (disabled until a narrow planner transport exists), DuckDuckGo URL-only safe-search starts, bounded source/evidence ledger, sensitive candidate classifier + blocked-action explanations, honest `insufficient_evidence` / `budget_without_evidence` reporting / **Slice 3** pause, take over, resume + evidence summary in chat (no arbitrary selectors, no search-box typing, no approval-to-submit path); not Playwright-in-process / not `/api/browser`); **GoHAM Mode** v0 (workspace **chat**, desktop shell only): user opt-in **observe/summarize** trail over the same real-browser IPC — not Shop-triggered, not “HAM Agent Mode” product naming; persisted `policy.json` (v3) + redacted audit JSONL; `docs/desktop/local_control_sidecar_protocol_v1.md`; `docs/desktop/local_control_v1.md`; `docs/goham/browser_smoke.md`) — no bundled API / PTY for local control |
