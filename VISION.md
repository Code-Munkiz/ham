...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 4B.1 Reactive Inbox Discovery:** Phase 2B remains execution-disconnected, Phase 3A `goham_controller.py` remains dry-run-only, and Phase 3B `goham_live_controller.py` remains original-post-only. Phase 4A `goham_reactive.py` still classifies prepared/read-only inbound mentions/comments into dry-run review/exception records. Phase 4B `reactive_reply_executor.py` / `goham_reactive_live.py` remain a separate one-shot reply canary. Phase 4B.1 adds `goham_reactive_inbox.py` plus Bearer-read normalization in `inbound_client.py` to discover mentions/comments, mark handled `goham_reactive_reply` journal rows, run policy/governor checks, and return at most one automatic `reply_target_id` without executing. Reactive budgets remain separate from broadcast caps; no scheduler, daemon, batch loop, original posts, quotes, DMs, likes/follows, xurl mutation, manual canary, broadcast executor, or Phase 2B execution |
