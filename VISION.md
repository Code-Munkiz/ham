...
| Workspace UI | `frontend/` (Vite + React), `desktop/` (Electron shell) | Extracted workspace; TypeScript types aligned with persisted run / bridge shapes; optional **Clerk** for chat JWT. **Desktop** is a thin Electron wrapper (`desktop/README.md`): **Windows** packaging via `npm run pack:win*`; **Linux AppImage/deb targets were removed.** `window.hamDesktop.localControl` exposes policy, audit, kill-switch, and inert **sidecar** status only — **shipped managed-browser / GoHAM chat execution IPC was removed**; use Ham **`/api/browser*`** for server-side browser runtime. **Settings → HAM + Hermes setup**: allowlisted Hermes **preset** IPC; dedicated **`POST /api/goham/planner`** remains a future-safe planning substrate (no desktop browser loop). See `docs/desktop/local_control_v1.md`, `docs/goham/browser_smoke.md` (future/retargeted). |
