...
| Workspace UI | `frontend/` (Vite + React), `desktop/` (Electron M1 shell) | Extracted workspace; TypeScript types aligned with persisted run / bridge shapes; optional **Clerk** (`VITE_CLERK_PUBLISHABLE_KEY`) for chat session JWT + `X-Ham-Operator-Authorization` for HAM confirm tokens; **desktop** is a thin Electron wrapper (`desktop/README.md`) with runtime `HAM_DESKTOP_API_BASE` / preload-injected config; **Settings → HAM + Hermes setup** offers allowlisted `hermes` **preset** runs (`runHermesPreset` IPC) plus **Local Control Phase 2–3B** (`window.hamDesktop.localControl`: status, policy/audit/kill-switch IPC, inert **sidecar** lifecycle IPC — `getSidecarStatus`, `pingSidecarHealth`, `stopSidecar`, `startSidecar` blocked by default kill switch; persisted `policy.json` + redacted audit JSONL; sidecar protocol `docs/desktop/local_control_sidecar_protocol_v1.md`; **disabled by default**; `docs/desktop/local_control_v1.md`) — no bundled API / PTY; sidecar is inert stdio shell only (no automation tools) |
