...
| HAM-on-X social agent | `src/ham/ham_x/`, `docs/ham-x-agent/` | **Phase 2B:** live-read/live-model dry-run path reads X with direct Bearer search, drafts with gated xAI, applies safety/autonomy routing, and writes review/exception/audit records with `execution_allowed=false` / `mutation_attempted=false`; separate Phase 2A manual canary executor remains gated/manual-only for one `post` or `quote` and is not called by dry-run, scoring, autonomy, smoke, or queues |
