...
**Managed mission selection (chat):** When replies need a **managed** `ManagedMission` row (status, repo/ref in operator payloads), the server scopes candidates with the chat **`project_id`**: prefer missions whose `control_plane_ham_run_id` resolves to a `ControlPlaneRun` with that `project_id`, else missions whose observed repository matches the bound project’s `metadata.cursor_cloud_repository` when set; if nothing matches, it falls back to any **open** mission or the newest row in a bounded recent list. Operators with multiple concurrent missions should keep **`project_id`** explicit in context. Details: `docs/ROADMAP_CLOUD_AGENT_MANAGED_MISSIONS.md` §2.
