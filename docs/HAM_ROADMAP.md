...
| **Phase 2G** | Creative media generation | Architecture: [`CREATIVE_MEDIA_GENERATION_INVESTIGATION.md`](CREATIVE_MEDIA_GENERATION_INVESTIGATION.md). **2G.1 (backend MVP) shipped:** text-to-image via `POST /api/media/images/generate`, opaque `hamgm_*` persistence, metadata + download routes, nested `generation` object on `GET /api/chat/capabilities`; provider path uses **OpenRouter** when `HAM_MEDIA_IMAGE_GENERATION_ENABLED` and plausible `OPENROUTER_API_KEY` are set. **2G.2 (workspace UI) shipped:** composer **+ → Generate image** + conservative NL routing for obvious create/make/draw prompts (no confirmation gate), preview/download via **`/api/media/artifacts/{id}`** and blob `fetch` (no browser→provider keys or raw `gs://` / storage paths in UI). **2G.3 (reference / image-conditioned generation) shipped:** optional `reference_attachment_id` on the same route (resolve bytes from **`AttachmentStore`** only server-side); capability-gated **`supports_image_to_image`** / **`supports_reference_images`**; NL + menu steer edit-style prompts when an image attachment is present; vision/analysis phrasing unchanged. Actual upstream **Flux / model** multimodal acceptance must be validated per deployment — disable with **`HAM_MEDIA_IMAGE_TO_IMAGE_ENABLED=false`** if unsupported. |
...
**Phase 2G (creative media generation):** **Image/video generation does not automatically follow** the user-selected **chat** model; HAM uses a **separate media capability layer** (generation vs `image_input` vs video **attachment** store-only). **Phase 2G.1** adds backend-only **text-to-image** (`/api/media/...`), `hamgm_*` storage (local dev or GCS with `HAM_GENERATED_MEDIA_*` / fallback bucket — see investigation doc §MVP notes), capability flags under JSON **`generation`**. **Phase 2G.2** adds the workspace chat **Generate image** action and NL routing. **Phase 2G.3** adds optional **reference attachment** conditioning (same ingest pipeline; **`hamatt_*`** only in the generation request — never browser→provider). **Phase 2G.4** adds wake-word/normalized NL routing when the composer still has attachments, workspace **copy** controls, generated-image card **remove-from-view**, and **`DELETE /api/chat/sessions/{id}`** for sidebar thread cleanup. **Phase 2G.5** adds **`src/ham/media_provider_registry.py`** — multi-backend selection via **`HAM_MEDIA_PROVIDER`** (OpenRouter remains the first live provider), placeholder metadata for future backends (OpenAI Images, Replicate, Runway, Luma, Kling), extended **`generation`** capability fields (`active_media_provider`, `available_media_providers`, conservative mode flags, **`provider_notes`**, no internal URLs), and **`docs/COMFYUI_PROVIDER_PLAN.md`**. **Phase 2G.6** ships **`src/ham/comfyui_provider_adapter.py`**: **`HAM_MEDIA_PROVIDER=comfyui`** calls a **remote** ComfyUI HTTP API (**`POST /prompt`**, **`/history`**, **`/view`**) using the tracked **`configs/media/comfyui`** **SDXL baseline** manifest/workflow template (placeholders only — **no** weights in-repo); **`supports_image_generation`** for Comfy is **true** only when **`HAM_COMFYUI_BASE_URL`** and the generation flag are set; **reference / image-to-image via Comfy** remains **unsupported** until a later slice. **Phase 2G.7** extends planning and ergonomics: **[`docs/COMFYUI_WORKER_TARGETS.md`](COMFYUI_WORKER_TARGETS.md)**, **`configs/media/comfyui/worker_targets.example.json`**, allowlisted **`HAM_COMFYUI_WORKER_PROFILE`** (optional **`generation.comfy_worker_profile`** echo), **`sdxl_vanilla` workflow alias**. **Phase 2G.10 (UI MVP)** adds explicit **`+ → Generate video`** with backend-job polling and generated video card/download; natural-language video routing stays deferred (**no** NL auto-route to video generation in this MVP). **Phase 2G.12 (local AnimateDiff POC)** adds **`configs/media/comfyui/animatediff_sdxl_gen1_mp4.*`** (**true-motion**) with **`HAM_COMFYUI_VIDEO_WORKFLOW`** switching vs **`comfy_video_local_poc`** repeated-frame fallback; **`ComfyUI-AnimateDiff-Evolved`** install and checkpoints remain **outside** this repo (**local/worker-operator** obligation). **Reachable-local-GPU**, **staging Cloud Run**, **GCS byte proof**, **Vercel UI proof**, and **`SEARGE_SDXL_*`** maturity are **manual/lab-network** checkpoints — shared production backends stay **Comfy-disabled** until infra signs off; **`ham-api`** remains HTTP-only (**no Torch/GPU** in-container).

**Historical — pre-implementation investigation text:** baseline audit referenced **no** dedicated generated-media store (now added for **2G.1**); OpenRouter documents **image** output via chat-completions **modalities** and **async video** via `/api/v1/videos` + poll — full research context remains in **`docs/CREATIVE_MEDIA_GENERATION_INVESTIGATION.md`**.
