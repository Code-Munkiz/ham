...
| **Game Pack** | [game-pack/](game-pack/) — **9 recipes** (all routed when flag on), **192 modules** |
...
All nine routed renders are under the 12k default budget.
...
- **No default v2 routing** — flag off by default; all nine current Game Pack recipes are routed when flag is on. **No current Game Pack recipes are schema-only anymore.** Future recipes still start schema-only until explicitly approved for routing.
...
1. **Choose the next Wave 2 recipe candidate**
2. **Consider `game.word-builder` for a safe word-family extension**
