...
- **Website Pack has three recipes** — routed behind flag: **`site.landing-page-core`**, **`site.dashboard-ui-core`**, **`app.saas-dashboard-core`**. **97 modules** total under `website-pack/`; v1 default preserved when flag is off; no templates or starter source files.
...
- **Website routing lanes (`site.landing-page-core` + `site.dashboard-ui-core` + `app.saas-dashboard-core`)** — all are narrowly routed behind `HAM_BUILD_REGISTRY_V2_ENABLED`; `app.saas-dashboard-core` matches only strong bounded SaaS app-home prompts with strict exclusions.
...
| **Website Pack** | [website-pack/](website-pack/) — **3 recipes**: `site.landing-page-core` (routed), `site.dashboard-ui-core` (routed), `app.saas-dashboard-core` (routed behind flag); **97 modules** |
...
Routed recipe coverage remains: all sixteen Game Pack recipes plus `site.landing-page-core`, `site.dashboard-ui-core`, and `app.saas-dashboard-core` behind `HAM_BUILD_REGISTRY_V2_ENABLED`. v1 remains default when the flag is off.
...
- **No default v2 routing** — flag off by default; routed when flag is on: all sixteen Game Pack recipes plus `site.landing-page-core`, `site.dashboard-ui-core`, and `app.saas-dashboard-core`.
...
- **`select_registry_v2_app_type_for_prompt(prompt)`** — pure regex; returns a game-pack app type id, routed website-pack app types (`site.landing-page-core`, `site.dashboard-ui-core`, or `app.saas-dashboard-core`), or `None`.
...
- **Routed app types today:** all sixteen Game Pack recipes (precedence: trivia → idle → … → city builder lite) plus **`site.landing-page-core`**, then **`site.dashboard-ui-core`**, then **`app.saas-dashboard-core`**.
