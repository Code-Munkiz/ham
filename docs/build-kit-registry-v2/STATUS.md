...
- **Fifteen Game Pack recipes are narrowly routable** behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches idle/incremental/clicker/tycoon, timed trivia/quiz game, branching/choice/story, memory card matching, daily word guessing / Wordle-style patterns, daily/grid/logic puzzle patterns, resource-management simulation game patterns, hangman / hidden-word / letter-guessing patterns, typing speed / WPM / typing challenge patterns, word-building / spelling / letter-pool patterns, turn-based card deck battle patterns (draw/hand/discard/turn/card-play), reaction-time challenge patterns (wait/signal/false-start/reaction-ms), rhythm tap timing game patterns (beat/cue/timing-window/perfect-good-miss/combo/streak), deck-building card game patterns (starter deck/encounter/reward/deck-mutation), or turn-based tactics grid skirmish patterns (grid/units/select/move/attack/player-enemy turns/win). **Wave 4 routed recipe:** **`game.turn-based-tactics-lite`**. **Wave 4 candidate recipe (schema-only, not routed):** **`game.city-builder-lite`** — DOM grid city builder with building placement, resource pools, day/tick production, population/happiness-lite, goal/result, and restart. See [WAVE_4_STRATEGY_SIM_DIRECTION.md](WAVE_4_STRATEGY_SIM_DIRECTION.md), [CITY_BUILDER_LITE_READINESS_REVIEW.md](CITY_BUILDER_LITE_READINESS_REVIEW.md), [TACTICS_GRID_AMBIGUITY_REVIEW.md](TACTICS_GRID_AMBIGUITY_REVIEW.md), [CARD_DECK_AMBIGUITY_REVIEW.md](CARD_DECK_AMBIGUITY_REVIEW.md), [DECK_BUILDER_LITE_READINESS_REVIEW.md](DECK_BUILDER_LITE_READINESS_REVIEW.md), [WAVE_3_POST_QUALITY_REPAIR_CHECKPOINT.md](WAVE_3_POST_QUALITY_REPAIR_CHECKPOINT.md), and [WAVE_3_PROGRESS_CHECKPOINT.md](WAVE_3_PROGRESS_CHECKPOINT.md).
...
- **Schema-only Wave 4 land for `game.city-builder-lite`** — recipe validates and composes; **not routed** behind `HAM_BUILD_REGISTRY_V2_ENABLED`; no API, frontend, Builder Studio, scaffold behavior, v1 JSON, template, or runtime changes in this step.
...
| **Game Pack** | [game-pack/](game-pack/) — **16 recipes** (15 routed when flag on; **`game.city-builder-lite` schema-only**), **376 modules** |
...
| **Tests** | `tests/test_build_registry.py` (99 cases), `tests/test_build_registry_scaffold_context.py`, `tests/test_builder_llm_scaffold_registry_context.py`, `tests/test_builder_llm_scaffold_registry_manual_smoke.py`, `tests/test_build_registry_intent.py`, `tests/test_build_registry_reference_checker.py` (12 cases) |
...
| `game.city-builder-lite` | Validated | **No** | Not routed — schema/docs only; routing requires explicit approval + tests | ~8.9k chars | Wave 4 candidate DOM city builder: grid placement, building palette, resource pools, day/tick production, population/happiness-lite, goal/result, restart; dashboard/resource-sim-without-placement/map-editor/factory/RTS/tactics/puzzle excluded; v1 fallback preserved |

Sixteen recipe renders target the 12k default budget. Fifteen Game Pack recipes are routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches. **`game.city-builder-lite` is schema-only — not routed.** v1 remains default when the flag is off.
...
- **No default v2 routing** — flag off by default; all fifteen Game Pack recipes are routed when flag is on. Future recipes still start schema-only until explicitly approved for routing. Recipe creation still does not imply routing.
...
- **Routed app types today:** `game.idle-incremental`, `game.trivia-timer`, `game.branching-narrative`, `game.memory-match`, `game.word-daily`, `game.daily-puzzle-grid`, `game.resource-management-sim`, `game.hangman-lite`, `game.typing-speed-racer`, `game.word-builder`, `game.card-deck-turn-based`, `game.reaction-time-challenge`, `game.rhythm-tap-lite`, `game.deck-builder-lite`, and `game.turn-based-tactics-lite` (precedence: trivia → idle → branching narrative → memory match → word daily → daily puzzle grid → resource management sim → hangman lite → typing speed racer → word builder → card deck turn-based → reaction time challenge → rhythm tap lite → deck builder lite → turn-based tactics lite).
