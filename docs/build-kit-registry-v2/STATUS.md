...
- **Routing-only Wave 4 land for `game.city-builder-lite`** — narrow intent routing + tests behind `HAM_BUILD_REGISTRY_V2_ENABLED`; no API, frontend, Builder Studio, scaffold behavior, v1 JSON, template, or recipe YAML changes in this step.
...
All sixteen recipe renders target the 12k default budget. All sixteen Game Pack recipes are routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches. v1 remains default when the flag is off.
