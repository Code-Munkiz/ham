...
- **Trivia, branching narrative, and memory match recipes are schema-only** — validated YAML that composes and renders; not wired to chat routing.
...
| **Game Pack** | [game-pack/](game-pack/) — **4 recipes**, **71 modules** |
...
| **Tests** | `tests/test_build_registry.py` (23 cases), `tests/test_build_registry_scaffold_context.py`, `tests/test_builder_llm_scaffold_registry_context.py`, `tests/test_builder_llm_scaffold_registry_manual_smoke.py`, `tests/test_build_registry_intent.py` |
...
- **Non-idle prompts remain v1** — SaaS, dashboard, generic, trivia, branching narrative, memory match, and ambiguous prompts do not get v2 metadata from routing today.
...
- **No default v2 routing** — flag off by default; trivia, branching narrative, and memory match not routed.
...
- **`game.trivia-timer`, `game.branching-narrative`, and `game.memory-match` are schema-only** — validate and compose in tests/CLI; no intent-router wiring yet.
...
1. **Add flag-gated routing** for `game.trivia-timer`, `game.branching-narrative`, or `game.memory-match` only after explicit approval (do not bundle with schema-only landings).
2. **Add `game.word-daily`** as recipe #5 following the Authoring Guide.
3. **Consider a routing strategy doc** before routing more recipes (intent patterns, negative patterns, approval checklist).
4. **Consider making CI registry validation blocking** once v2 usage increases (today warning-only for idle app type + registry tests).
5. **Add manual outcome report examples** for non-idle recipes (idle success example exists under [examples/outcome-facts/](examples/outcome-facts/)).
