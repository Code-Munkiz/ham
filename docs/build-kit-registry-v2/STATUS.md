...
- **Game Pack has fifteen recipes** — **349 indexed modules** total.
- **Fourteen Game Pack recipes are narrowly routable** behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches idle/incremental/clicker/tycoon, timed trivia/quiz game, branching/choice/story, memory card matching, daily word guessing / Wordle-style patterns, daily/grid/logic puzzle patterns, resource-management simulation game patterns, hangman / hidden-word / letter-guessing patterns, typing speed / WPM / typing challenge patterns, word-building / spelling / letter-pool patterns, turn-based card deck battle patterns (draw/hand/discard/turn/card-play), reaction-time challenge patterns (wait/signal/false-start/reaction-ms), or rhythm tap timing game patterns (beat/cue/timing-window/perfect-good-miss/combo/streak). **Wave 2 includes** **`game.daily-puzzle-grid`**, **`game.resource-management-sim`**, **`game.hangman-lite`**, **`game.typing-speed-racer`**, and **`game.word-builder`**. **Wave 3 routed recipes include** **`game.card-deck-turn-based`**, **`game.reaction-time-challenge`**, **`game.rhythm-tap-lite`**, and **`game.deck-builder-lite`**. **Wave 4 schema-only (not routed):** **`game.turn-based-tactics-lite`** — DOM grid tactics skirmish with unit selection, move/attack ranges, player/enemy turns, and battle result/restart. See [WAVE_4_STRATEGY_SIM_DIRECTION.md](WAVE_4_STRATEGY_SIM_DIRECTION.md), [TACTICS_GRID_AMBIGUITY_REVIEW.md](TACTICS_GRID_AMBIGUITY_REVIEW.md), [CARD_DECK_AMBIGUITY_REVIEW.md](CARD_DECK_AMBIGUITY_REVIEW.md), [DECK_BUILDER_LITE_READINESS_REVIEW.md](DECK_BUILDER_LITE_READINESS_REVIEW.md), [WAVE_3_POST_QUALITY_REPAIR_CHECKPOINT.md](WAVE_3_POST_QUALITY_REPAIR_CHECKPOINT.md), and [WAVE_3_PROGRESS_CHECKPOINT.md](WAVE_3_PROGRESS_CHECKPOINT.md).
...
- **Local reference checker exists** — `scripts/check_build_registry_references.py` audits pack references, duplicates, orphans, and render-budget headroom. **Local/manual only — not CI-blocking.** Current registry (15 recipes / 349 modules): run checker after schema land. See [REFERENCE_CHECKER_IMPLEMENTATION_PLAN.md](REFERENCE_CHECKER_IMPLEMENTATION_PLAN.md) and [WAVE_3_COMPLETION_CHECKPOINT.md](WAVE_3_COMPLETION_CHECKPOINT.md).
- **No runtime, API, frontend, Builder Studio, scaffold, or routing changes** from Wave 4 schema-only land — **`game.turn-based-tactics-lite` is not routed** until explicit approval and tests.
...
| **Game Pack** | [game-pack/](game-pack/) — **15 recipes** (14 routed when flag on; **1 schema-only**), **349 modules** |
...
| **Tests** | `tests/test_build_registry.py` (92 cases), `tests/test_build_registry_scaffold_context.py`, `tests/test_builder_llm_scaffold_registry_context.py`, `tests/test_builder_llm_scaffold_registry_manual_smoke.py`, `tests/test_build_registry_intent.py`, `tests/test_build_registry_reference_checker.py` (12 cases) |
...
| `game.turn-based-tactics-lite` | Validated | **No** | Not routed — schema-only | ~8.5k chars | Wave 4 DOM grid tactics skirmish; unit select/move/attack, player/enemy turns, battle result/restart; chess/puzzle-grid/city-builder/resource-sim/tower-defense/RTS/RPG/map-editor/physics/card-battle excluded; routing requires explicit approval |

Fifteen recipe renders target the 12k default budget. Fourteen Game Pack recipes are routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches; **`game.turn-based-tactics-lite` is schema-only and not routed**. v1 remains default when the flag is off.
...
- **No default v2 routing** — flag off by default; all fourteen Game Pack recipes are routed when flag is on. Future recipes still start schema-only until explicitly approved for routing. Recipe creation still does not imply routing.
...
- **`select_registry_v2_app_type_for_prompt(prompt)`** — pure regex; returns `game.idle-incremental`, `game.trivia-timer`, `game.branching-narrative`, `game.memory-match`, `game.word-daily`, `game.daily-puzzle-grid`, `game.resource-management-sim`, `game.hangman-lite`, `game.typing-speed-racer`, `game.word-builder`, `game.card-deck-turn-based`, `game.reaction-time-challenge`, `game.rhythm-tap-lite`, `game.deck-builder-lite`, or `None`.
...
- **Routed app types today:** `game.idle-incremental`, `game.trivia-timer`, `game.branching-narrative`, `game.memory-match`, `game.word-daily`, `game.daily-puzzle-grid`, `game.resource-management-sim`, `game.hangman-lite`, `game.typing-speed-racer`, `game.word-builder`, `game.card-deck-turn-based`, `game.reaction-time-challenge`, `game.rhythm-tap-lite`, and `game.deck-builder-lite` (precedence: trivia → idle → branching narrative → memory match → word daily → daily puzzle grid → resource management sim → hangman lite → typing speed racer → word builder → card deck turn-based → reaction time challenge → rhythm tap lite → deck builder lite).
...
- **Deck builder lite routing is conservative** — requires deck-building card game signals with starter deck, encounters/battles, reward choices, and deck mutation/progression; lowest precedence after rhythm tap lite; pitch/slide/presentation decks, flashcard study decks, marketplace/NFT/trading cards, gambling/casino, dashboard/kanban/card layout, construction/project planning decks, music/audio decks, generic “deck builder” / “deck” / “cards” alone, and simple turn-based card battle-only prompts do not route to `game.deck-builder-lite`.
...
1. **Wave 4 routing approval for `game.turn-based-tactics-lite`** — schema landed; routing + generated gate review remain separate steps. See [TACTICS_GRID_AMBIGUITY_REVIEW.md](TACTICS_GRID_AMBIGUITY_REVIEW.md).
