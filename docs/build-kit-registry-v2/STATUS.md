...
- **Website Pack has five recipes** — routed behind flag: **`site.landing-page-core`**, **`site.dashboard-ui-core`**, **`app.saas-dashboard-core`**, **`app.admin-dashboard-core`**, **`app.sales-ops-dashboard-core`**. **188 modules** total under `website-pack/`; v1 default preserved when flag is off; no templates or starter source files.
...
- **Website routing lanes (`site.landing-page-core` + `site.dashboard-ui-core` + `app.saas-dashboard-core` + `app.admin-dashboard-core`)** — all are narrowly routed behind `HAM_BUILD_REGISTRY_V2_ENABLED`; `app.saas-dashboard-core` remains strong bounded SaaS app-home intent only, and `app.admin-dashboard-core` remains strong bounded static admin-control-surface intent only.
...
| **Website Pack** | [website-pack/](website-pack/) — **5 recipes**: `site.landing-page-core` (routed), `site.dashboard-ui-core` (routed), `app.saas-dashboard-core` (routed behind flag), `app.admin-dashboard-core` (routed behind flag), `app.sales-ops-dashboard-core` (routed behind flag); **188 modules** |
...
| `app.admin-dashboard-core` | Validated | Yes (narrow) | `HAM_BUILD_REGISTRY_V2_ENABLED` + strong bounded static admin-control-surface intent (admin dashboard/control-panel + admin regions + static/demo/local-mock + no-backend/no-auth/no-rbac/no-crud/no-destructive/no-live constraints; strict exclusions for real auth/backend/RBAC/CRUD/destructive/live-monitoring/billing/security/compliance implementation/analytics-workbench/CRM/ecommerce/fintech/clones) | ~11k chars target | Static admin shell preview: sidebar/topbar, overview status cards, user/team and role/permission summaries, review queue, resource table, audit/activity list, system status, demo-mode controls, optional non-mutating danger modal, and empty/loading/error examples; strict exclusions for auth/backend/RBAC/CRUD/destructive/live-monitoring/billing/security implementation/clones. |
| `app.sales-ops-dashboard-core` | Validated | Yes (narrow) | `HAM_BUILD_REGISTRY_V2_ENABLED` + strong bounded sales-ops/revenue-recovery intent (sales-ops dashboard + domain regions + static/local/demo/no-backend/no-payroll/no-payments/no-accounting/no-CRM/legal-automation constraints; excludes generic dashboards, SaaS/admin homes, payroll/payment/accounting/ASC606/legal collections automation, live CRM/backend/API/database, PII, trading, clone prompts) | ~11k chars target | Static sales/revenue-operations preview: shell, executive summary, agent/team performance, sales activity, pipeline movement, commission summary + payout status display, recovery summary + aging buckets + exception queue, process bottleneck panel, activity feed, filters, empty/loading/error states, semantic financial table, and coherent local sample data; strict exclusions for payroll/payment/accounting/ASC 606/legal collections automation/CRM sync/backend/PII/compliance claims/trading/regulated financial advice. |

Routed recipe coverage remains: all sixteen Game Pack recipes plus `site.landing-page-core`, `site.dashboard-ui-core`, `app.saas-dashboard-core`, `app.admin-dashboard-core`, and `app.sales-ops-dashboard-core` behind `HAM_BUILD_REGISTRY_V2_ENABLED`. v1 remains default when the flag is off.
...
- **Flag on + bounded static admin control-surface prompt:** routing adds `registry_v2_app_type: app.admin-dashboard-core` only when prompts combine strong admin-dashboard/control-panel framing + admin-domain regions (user/team summary, role/permission summary, review/moderation queue, audit log, system status, resource/user table) + explicit static/demo/no-backend/no-auth/no-rbac/no-crud/no-destructive/no-live constraints. Weak generic terms are excluded.
...
- **No default v2 routing** — flag off by default; routed when flag is on: all sixteen Game Pack recipes plus `site.landing-page-core`, `site.dashboard-ui-core`, `app.saas-dashboard-core`, and `app.admin-dashboard-core`.
...
- **`select_registry_v2_app_type_for_prompt(prompt)`** — pure regex; returns a game-pack app type id, routed website-pack app types (`site.landing-page-core`, `site.dashboard-ui-core`, `app.saas-dashboard-core`, or `app.admin-dashboard-core`), or `None`.
...
- **Routed app types today:** all sixteen Game Pack recipes (precedence: trivia → idle → … → city builder lite) plus **`site.landing-page-core`**, then **`site.dashboard-ui-core`**, then **`app.saas-dashboard-core`**, then **`app.admin-dashboard-core`**.
