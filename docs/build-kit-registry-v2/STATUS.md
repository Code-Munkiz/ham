...
- **Website Pack has two recipes** — **`site.landing-page-core`** (routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches static landing/marketing page signals) and **`site.dashboard-ui-core`** (read-only dashboard overview, **schema-only — not routed**). **59 modules** total under `website-pack/`; v1 default preserved when flag is off; no templates or starter source files.
...
- **Routing-only website lane for `site.landing-page-core`** — narrow landing-page intent routing + tests behind `HAM_BUILD_REGISTRY_V2_ENABLED`; no generic website/homepage/page/design router; no API, frontend, Builder Studio, scaffold behavior, v1 JSON, template, or recipe YAML changes in this step.
...
| **Website Pack** | [website-pack/](website-pack/) — **2 recipes**: `site.landing-page-core` (routed when flag on + narrow landing intent) and `site.dashboard-ui-core` (read-only dashboard, **schema-only, not routed**); **59 modules** |
...
| `site.dashboard-ui-core` | Validated | **No (schema-only)** | Not routed — no `intent.py` wiring | ~10.8k chars | Read-only / mostly static dashboard overview: dashboard shell, bounded 3–5 KPI row, 1–2 basic charts, simple table, optional local filters, empty/loading/error states, responsive + a11y. No backend/auth/CRUD/payments/admin/analytics-workbench/real-time/maps/trading/game-HUD. Routing deferred per [DASHBOARD_UI_CORE_READINESS_REVIEW.md](DASHBOARD_UI_CORE_READINESS_REVIEW.md) |

All eighteen recipe renders target the 12k default budget. All sixteen Game Pack recipes plus **`site.landing-page-core`** are routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches; **`site.dashboard-ui-core`** is **schema-only and not routed**. v1 remains default when the flag is off.
...
- **No default v2 routing** — flag off by default; all sixteen Game Pack recipes plus **`site.landing-page-core`** are routed when flag is on. Future recipes still start schema-only until explicitly approved for routing. Recipe creation still does not imply routing.
...
- **`select_registry_v2_app_type_for_prompt(prompt)`** — pure regex; returns a game-pack app type id, `site.landing-page-core`, or `None`.
...
- **Routed app types today:** all sixteen Game Pack recipes (precedence: trivia → idle → … → city builder lite) plus **`site.landing-page-core`** (lowest precedence; after all game checks).
