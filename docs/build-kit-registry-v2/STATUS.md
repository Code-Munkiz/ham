...
- **Website Pack has three recipes** — routed: **`site.landing-page-core`**, **`site.dashboard-ui-core`**; schema-only/not routed: **`app.saas-dashboard-core`**. **97 modules** total under `website-pack/`; v1 default preserved when flag is off; no templates or starter source files.
...
- **Website routing lanes (`site.landing-page-core` + `site.dashboard-ui-core`)** — both are narrowly routed behind `HAM_BUILD_REGISTRY_V2_ENABLED`; `app.saas-dashboard-core` remains schema-only and intentionally not routed in this step.
...
| **Website Pack** | [website-pack/](website-pack/) — **3 recipes**: `site.landing-page-core` (routed), `site.dashboard-ui-core` (routed), `app.saas-dashboard-core` (schema-only, not routed); **97 modules** |
...
| `app.saas-dashboard-core` | Validated | No (schema-only) | Deferred by design (not wired in `intent.py`; `resolve_pack_root` mapping deferred) | ~11k chars target | Static app-shell-light SaaS product home: workspace context placeholder, usage cards, plan/status, activity feed, resource list, single upgrade CTA, settings/help shortcuts, and empty/loading/error states; strict exclusions for auth/backend/billing/admin/RBAC/CRUD/analytics-workbench/realtime/fintech/ecommerce/maps/clones. |

Routed recipe coverage remains: all sixteen Game Pack recipes plus `site.landing-page-core` and `site.dashboard-ui-core` behind `HAM_BUILD_REGISTRY_V2_ENABLED`. `app.saas-dashboard-core` is intentionally schema-only and not routed yet. v1 remains default when the flag is off.
...
- **No default v2 routing** — flag off by default; routed when flag is on: all sixteen Game Pack recipes plus `site.landing-page-core` and `site.dashboard-ui-core`. `app.saas-dashboard-core` remains schema-only until explicit routing approval.
...
- **`select_registry_v2_app_type_for_prompt(prompt)`** — pure regex; returns a game-pack app type id, routed website-pack app types (`site.landing-page-core` or `site.dashboard-ui-core`), or `None` (`app.saas-dashboard-core` not wired yet).
...
- **Routed app types today:** all sixteen Game Pack recipes (precedence: trivia → idle → … → city builder lite) plus **`site.landing-page-core`** then **`site.dashboard-ui-core`**. **`app.saas-dashboard-core` is schema-only and intentionally unrouted.**
