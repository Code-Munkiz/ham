...
- **Routing-only Wave 4 land for `game.turn-based-tactics-lite`** — narrow intent routing + tests behind `HAM_BUILD_REGISTRY_V2_ENABLED`; no API, frontend, Builder Studio, scaffold behavior, v1 JSON, template, or recipe YAML changes in this step.
...
| **Game Pack** | [game-pack/](game-pack/) — **15 recipes** (all routed when flag on), **349 modules** |
...
Fifteen recipe renders target the 12k default budget. All fifteen Game Pack recipes are routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches. v1 remains default when the flag is off.
