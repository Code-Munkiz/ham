...
- **Website Pack has five recipes** — routed behind flag: **`site.landing-page-core`**, **`site.dashboard-ui-core`**, **`app.saas-dashboard-core`**, **`app.admin-dashboard-core`**; schema-only (not routed): **`app.sales-ops-dashboard-core`**. **188 modules** total under `website-pack/`; v1 default preserved when flag is off; no templates or starter source files.
...
| **Website Pack** | [website-pack/](website-pack/) — **5 recipes**: `site.landing-page-core` (routed), `site.dashboard-ui-core` (routed), `app.saas-dashboard-core` (routed behind flag), `app.admin-dashboard-core` (routed behind flag), `app.sales-ops-dashboard-core` (schema-only, not routed); **188 modules** |
...
| `app.sales-ops-dashboard-core` | Validated | No (schema-only) | N/A (not routed) | ~11k chars target | Static sales/revenue-operations preview: shell, executive summary, agent/team performance, sales activity, pipeline movement, commission summary + payout status display, recovery summary + aging buckets + exception queue, process bottleneck panel, activity feed, filters, empty/loading/error states, semantic financial table, and coherent local sample data; strict exclusions for payroll/payment/accounting/ASC 606/legal collections automation/CRM sync/backend/PII/compliance claims/trading/regulated financial advice. |

Routed recipe coverage remains: all sixteen Game Pack recipes plus `site.landing-page-core`, `site.dashboard-ui-core`, `app.saas-dashboard-core`, and `app.admin-dashboard-core` behind `HAM_BUILD_REGISTRY_V2_ENABLED`. `app.sales-ops-dashboard-core` is schema-only and not routed. v1 remains default when the flag is off.
