...
- **Website Pack has two recipes** — **`site.landing-page-core`** and **`site.dashboard-ui-core`**, both routed narrowly behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches their bounded lanes (landing/marketing vs read-only dashboard overview). **59 modules** total under `website-pack/`; v1 default preserved when flag is off; no templates or starter source files.
...
- **Website routing lanes (`site.landing-page-core` + `site.dashboard-ui-core`)** — both are narrowly routed behind `HAM_BUILD_REGISTRY_V2_ENABLED`; landing keeps strict marketing-page signals, dashboard keeps strict read-only overview + KPI/chart/table signals and broad exclusions; no API, frontend, Builder Studio, scaffold behavior, v1 JSON, template, or recipe YAML changes in this step.
...
| **Website Pack** | [website-pack/](website-pack/) — **2 recipes**: `site.landing-page-core` (routed when flag on + narrow landing intent) and `site.dashboard-ui-core` (routed when flag on + narrow read-only dashboard intent); **59 modules** |
...
| `site.dashboard-ui-core` | Validated | Yes (narrow) | `HAM_BUILD_REGISTRY_V2_ENABLED` + strict read-only dashboard intent (overview + KPI cards + charts + table; exclusions for admin/analytics-workbench/backend-auth/CRUD/realtime/maps/trading/game-HUD/landing-screenshot) | ~10.8k chars | Read-only / mostly static dashboard overview: dashboard shell, bounded 3–5 KPI row, 1–2 basic charts, simple table, optional local filters, empty/loading/error states, responsive + a11y. No backend/auth/CRUD/payments/admin/analytics-workbench/real-time/maps/trading/game-HUD. |

All eighteen recipe renders target the 12k default budget. All sixteen Game Pack recipes plus both Website Pack recipes are routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches their bounded lanes. v1 remains default when the flag is off.
...
- **No default v2 routing** — flag off by default; all sixteen Game Pack recipes plus both Website Pack recipes are routed when flag is on. Future recipes still start schema-only until explicitly approved for routing. Recipe creation still does not imply routing.
...
- **`select_registry_v2_app_type_for_prompt(prompt)`** — pure regex; returns a game-pack app type id, a routed website-pack app type (`site.landing-page-core` or `site.dashboard-ui-core`), or `None`.
...
- **Routed app types today:** all sixteen Game Pack recipes (precedence: trivia → idle → … → city builder lite) plus **`site.landing-page-core`** then **`site.dashboard-ui-core`** (dashboard remains low-precedence and only on strict combined read-only dashboard signals).
