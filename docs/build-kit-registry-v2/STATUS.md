...
- **Website Pack skeleton landed** — **`site.landing-page-core`** authored under `website-pack/` (**29 modules**); **narrowly routed** behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches static landing/marketing page signals; v1 default preserved when flag is off; no templates or starter source files.
...
| **Website Pack** | [website-pack/](website-pack/) — **1 recipe** (`site.landing-page-core`, routed when flag on + narrow landing intent), **29 modules** |
...
| `site.landing-page-core` | Validated | Yes (narrow) | `HAM_BUILD_REGISTRY_V2_ENABLED` + narrow static landing/marketing page intent | ~8.6k chars | Conservative combined landing-page/hero/features/value-proposition/CTA/social-proof/FAQ/final-CTA routing; dashboard/ecommerce/CMS/backend/auth/full-web-app/game/clone/generic website/homepage/page/design prompts excluded; v1 fallback preserved |

All seventeen recipe renders target the 12k default budget. All sixteen Game Pack recipes plus **`site.landing-page-core`** are routed behind `HAM_BUILD_REGISTRY_V2_ENABLED` when prompt intent clearly matches. v1 remains default when the flag is off.
