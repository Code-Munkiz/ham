...
| **UI** | **Partial:** managed mission APIs and operator/chat flows remain; dedicated Cloud Agent / War Room panels were removed with legacy workbench (Batch 2A). Hermes Workspace **Live Cloud Agent missions** renders the feed as a **transcript** (folded assistant / thinking / tool / status / user) via `missionFeedTranscript.ts` + `WorkspaceManagedMissionsLivePanel`. Re-home any remaining mission UX in Workspace or Command Center as needed. |
