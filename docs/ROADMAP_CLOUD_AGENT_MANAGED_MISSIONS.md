...
**Related:** `GAPS.md`, `VISION.md` (transitional state table), `docs/CONTROL_PLANE_RUN.md` (factual run records vs. mission graph), `docs/HAM_CLOUD_AGENT_ROUTING_SMOKE.md` (Workspace Chat uses `POST /api/chat/stream` and operator/router short-circuit before LLM tokens).
