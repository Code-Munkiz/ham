...
| **Read API: mission feed** | `GET /api/cursor/managed/missions/{mission_registry_id}/feed` returns status, repo/ref, artifacts, and a bounded **`events`** list (last **80** stored rows, or a small synthetic timeline when none are persisted yet). On each **GET**, the server best-effort **refreshes** the Cursor agent payload and **maps** the agent conversation into additional `mission_feed_events` when an effective **Cursor API key** is present; the JSON includes `provider_projection_state` (`ok` \| `fallback`) and `provider_projection_reason` when enrichment is skipped or fails. **`POST .../missions/{id}/messages`** and **`POST .../missions/{id}/cancel`** append feed rows (follow-up / cancel outcomes). |
