...
- `audit_sink` only lists values that exist today on `ControlPlaneProviderAuditRef` (`cursor_jsonl` / `droid_jsonl`); **planned** Claude Code and OpenCode have no sink in that enum yet.
...
| `claude_code` | Claude Code | local_cli_planned | **planned_candidate** | **yes** (v1 intent) | no | no (v1 intent) | no (v1 intent) | no (v1 intent) | no (v1 intent) | no (v1 intent) | yes (v1 intent) | **none in CP yet** | TBD | TBD | TBD | **Not implemented in HAM** — no `ControlPlaneProvider` value; no runtime. The `claude_agent_sdk` row in `src/api/workspace_tools.py` is workspace-tool readiness, **not** a coding-agent control-plane provider. |
