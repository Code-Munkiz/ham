...
**Not source of truth:** generated exports and editor-local Cursor settings (for example [`CURSOR_EXACT_SETUP_EXPORT.md`](../CURSOR_EXACT_SETUP_EXPORT.md), personal files under `.cursor/` such as `settings.json` when not tracked) are snapshots or editor config—defer to git-tracked canonical docs unless you deliberately refresh an export script.
