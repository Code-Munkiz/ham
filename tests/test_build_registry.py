...
EXPECTED_WORD_BUILDER_MECHANIC_ORDER = (
    "mechanic.letter-pool",
    "mechanic.word-slot-construction",
    "mechanic.word-validation",
    "mechanic.submission-attempts",
    "mechanic.word-builder-scoring",
    "mechanic.hint-reveal-lite",
    "mechanic.level-progression",
...
EXPECTED_REACTION_TIME_CHALLENGE_MECHANIC_ORDER = (
    "mechanic.reaction-state-machine",
    "mechanic.random-signal-delay",
    "mechanic.false-start-handling",
    "mechanic.reaction-timer",
    "mechanic.reaction-input-response",
    "mechanic.reaction-time-scoring",
    "mechanic.reaction-round-progression",
...
EXPECTED_DECK_BUILDER_LITE_MECHANIC_ORDER = (
    "mechanic.deck-builder-run-state-machine",
    "mechanic.deck-builder-result-state",
    "mechanic.deck-draw-pile",
    "mechanic.hand-state",
    "mechanic.card-effect-resolution",
    "mechanic.discard-pile",
    "mechanic.encounter-round-loop",
    "mechanic.opponent-challenge-state",
    "mechanic.reward-offer-choice",
    "mechanic.starter-deck-seed",
...
EXPECTED_CITY_BUILDER_LITE_MECHANIC_ORDER = (
...
class TestWordBuilderRecipe:
    def test_compose_word_builder(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        validate_registry_pack(pack)
        recipe = compose_build_recipe(pack, "game.word-builder")
        assert recipe.app_type_id == "game.word-builder"
        assert recipe.stack_kit_id == "stack.dom-game-minimal"
        assert recipe.mechanic_ids == EXPECTED_WORD_BUILDER_MECHANIC_ORDER

    def test_render_word_builder_includes_key_ids(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.word-builder")
        rendered = render_playbook_context(recipe)
        assert "game.word-builder" in rendered
        assert "stack.dom-game-minimal" in rendered
        assert "mechanic.letter-pool" in rendered
        assert "mechanic.word-slot-construction" in rendered
        assert "mechanic.word-validation" in rendered
        assert "mechanic.word-builder-scoring" in rendered
        assert "validator.letter-pool-integrity" in rendered
        assert "validator.word-validation-rules" in rendered
        assert "validator.duplicate-submission-blocking" in rendered
        assert "validator.word-builder-completion" in rendered
        assert "static-word-list-for-mvp" in rendered

    def test_render_word_builder_under_default_budget(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.word-builder")
        rendered = render_playbook_context(recipe)
        assert len(rendered) <= 12_000
    ...
    def test_word_builder_adaptive_policy_fields(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        app = pack.module_data("game.word-builder")
        for field in WAVE_1_ADAPTIVE_POLICY_LIST_FIELDS:
            value = app.get(field)
            assert isinstance(value, list), f"game.word-builder: {field} must be a list"
            assert value, f"game.word-builder: {field} must be non-empty"
    ...
    def test_existing_nine_recipes_still_compose_after_word_builder_added(
        self, game_pack_root: Path
    ):
        pack = load_registry_pack(game_pack_root)
        validate_registry_pack(pack)
...
class TestReactionTimeChallengeRecipe:
    def test_app_type_exists_in_game_pack(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        assert "game.reaction-time-challenge" in pack.modules
        app = pack.module_data("game.reaction-time-challenge")
        assert app.get("kind") == "app_type"

    def test_compose_reaction_time_challenge(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        validate_registry_pack(pack)
        recipe = compose_build_recipe(pack, "game.reaction-time-challenge")
        assert recipe.app_type_id == "game.reaction-time-challenge"
        assert recipe.stack_kit_id == "stack.dom-game-minimal"
        assert recipe.mechanic_ids == EXPECTED_REACTION_TIME_CHALLENGE_MECHANIC_ORDER

    def test_render_reaction_time_includes_key_ids(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.reaction-time-challenge")
        rendered = render_playbook_context(recipe)
        assert "game.reaction-time-challenge" in rendered
        assert "stack.dom-game-minimal" in rendered
        assert "mechanic.reaction-state-machine" in rendered
        assert "mechanic.false-start-handling" in rendered
        assert "mechanic.reaction-timer" in rendered
        assert "mechanic.reaction-time-scoring" in rendered
        assert "validator.false-start-enforcement" in rendered
        assert "validator.reaction-timer-bounds" in rendered
        assert "no-medical-biometric-claims-for-mvp" in rendered
        assert "no-typing-speed-game-for-mvp" in rendered
        assert "no-productivity-timer-app-for-mvp" in rendered

    def test_render_reaction_time_under_default_budget(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.reaction-time-challenge")
        rendered = render_playbook_context(recipe)
        assert len(rendered) <= 12_000
    ...
    def test_reaction_time_adaptive_policy_fields(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        app = pack.module_data("game.reaction-time-challenge")
        for field in WAVE_1_ADAPTIVE_POLICY_LIST_FIELDS:
            value = app.get(field)
            assert isinstance(value, list), f"game.reaction-time-challenge: {field} must be a list"
            assert value, f"game.reaction-time-challenge: {field} must be non-empty"
...
class TestDeckBuilderLiteRecipe:
    def test_app_type_exists_in_game_pack(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        assert "game.deck-builder-lite" in pack.modules
        app = pack.module_data("game.deck-builder-lite")
        assert app.get("kind") == "app_type"

    def test_compose_deck_builder_lite(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        validate_registry_pack(pack)
        recipe = compose_build_recipe(pack, "game.deck-builder-lite")
        assert recipe.app_type_id == "game.deck-builder-lite"
        assert recipe.stack_kit_id == "stack.dom-game-minimal"
        assert recipe.mechanic_ids == EXPECTED_DECK_BUILDER_LITE_MECHANIC_ORDER

    def test_render_deck_builder_lite_includes_key_ids(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.deck-builder-lite")
        rendered = render_playbook_context(recipe)
        assert "game.deck-builder-lite" in rendered
        assert "stack.dom-game-minimal" in rendered
        assert "mechanic.starter-deck-seed" in rendered
        assert "mechanic.encounter-round-loop" in rendered
        assert "mechanic.reward-offer-choice" in rendered
        assert "mechanic.deck-mutation" in rendered
        assert "mechanic.deck-builder-result-state" in rendered
        assert "validator.starter-deck-non-empty" in rendered
        assert "validator.reward-choice-integrity" in rendered
        assert "validator.deck-mutation-consistency" in rendered
        assert "no-pitch-deck-or-presentation-for-mvp" in rendered
        assert "no-flashcard-study-deck-for-mvp" in rendered
        assert "no-map-pathing-campaign-for-mvp" in rendered

    def test_render_deck_builder_lite_under_default_budget(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.deck-builder-lite")
        rendered = render_playbook_context(recipe)
        assert len(rendered) <= 12_000
    ...
    def test_deck_builder_lite_adaptive_policy_fields(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        app = pack.module_data("game.deck-builder-lite")
        for field in WAVE_1_ADAPTIVE_POLICY_LIST_FIELDS:
            value = app.get(field)
            assert isinstance(value, list), f"game.deck-builder-lite: {field} must be a list"
            assert value, f"game.deck-builder-lite: {field} must be non-empty"
    ...
    def test_deck_builder_lite_routed_in_intent_module(self):
        import src.ham.build_registry.intent as intent_module

        source = Path(intent_module.__file__).read_text(encoding="utf-8")
        assert "game.deck-builder-lite" in source
        assert "DECK_BUILDER_LITE_APP_TYPE" in source
        assert "_matches_deck_builder_lite" in source

    def test_existing_thirteen_routed_recipes_still_compose_after_deck_builder_lite_added(
        self, game_pack_root: Path
    ):
        pack = load_registry_pack(game_pack_root)
        validate_registry_pack(pack)
...
class TestCityBuilderLiteRecipe:
    def test_app_type_exists_in_game_pack(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        assert "game.city-builder-lite" in pack.modules
        app = pack.module_data("game.city-builder-lite")
        assert app.get("kind") == "app_type"

    def test_compose_city_builder_lite(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        validate_registry_pack(pack)
        recipe = compose_build_recipe(pack, "game.city-builder-lite")
        assert recipe.app_type_id == "game.city-builder-lite"
        assert recipe.stack_kit_id == "stack.dom-game-minimal"
        assert recipe.mechanic_ids == EXPECTED_CITY_BUILDER_LITE_MECHANIC_ORDER

    def test_render_city_builder_lite_includes_key_ids(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.city-builder-lite")
        rendered = render_playbook_context(recipe)
        assert "game.city-builder-lite" in rendered
        assert "stack.dom-game-minimal" in rendered
        assert "mechanic.city-grid-state" in rendered
        assert "mechanic.city-production-tick" in rendered
        assert "mechanic.city-goal-result-state" in rendered
        assert "validator.city-seeded-grid" in rendered
        assert "validator.city-goal-result-detection" in rendered
        assert "no-map-editor-for-mvp" in rendered
        assert "no-resource-dashboard-for-mvp" in rendered
        assert "no-factory-automation-for-mvp" in rendered

    def test_render_city_builder_lite_under_default_budget(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        recipe = compose_build_recipe(pack, "game.city-builder-lite")
        rendered = render_playbook_context(recipe)
        assert len(rendered) <= 12_000
    ...
    def test_city_builder_lite_adaptive_policy_fields(self, game_pack_root: Path):
        pack = load_registry_pack(game_pack_root)
        app = pack.module_data("game.city-builder-lite")
        for field in WAVE_1_ADAPTIVE_POLICY_LIST_FIELDS:
            value = app.get(field)
            assert isinstance(value, list), f"game.city-builder-lite: {field} must be a list"
            assert value, f"game.city-builder-lite: {field} must be non-empty"
    ...
    def test_existing_fifteen_routed_recipes_still_compose_after_city_builder_lite_added(
        ...
        rhythm = compose_build_recipe(pack, "game.rhythm-tap-lite")
        deck_builder = compose_build_recipe(pack, "game.deck-builder-lite")
        tactics = compose_build_recipe(pack, "game.turn-based-tactics-lite")
        assert idle.mechanic_ids == EXPECTED_MECHANIC_ORDER
        assert trivia.mechanic_ids == EXPECTED_TRIVIA_MECHANIC_ORDER
        assert branching.mechanic_ids == EXPECTED_BRANCHING_NARRATIVE_MECHANIC_ORDER
        assert memory.mechanic_ids == EXPECTED_MEMORY_MATCH_MECHANIC_ORDER
        ...
        assert rhythm.mechanic_ids == EXPECTED_RHYTHM_TAP_LITE_MECHANIC_ORDER
        assert deck_builder.mechanic_ids == EXPECTED_DECK_BUILDER_LITE_MECHANIC_ORDER
        assert tactics.mechanic_ids == EXPECTED_TURN_BASED_TACTICS_LITE_MECHANIC_ORDER
