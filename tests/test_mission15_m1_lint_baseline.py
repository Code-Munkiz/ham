"""
Mission 15 M1 lint and format baseline verification.
...
Assertions covered:
  - VAL-M15-M1-LINT-RUFF-CHECK-001  (ruff check ≤ 732 lines)
  - VAL-M15-M1-LINT-RUFF-FORMAT-001 (ruff format-check ≤ 433 files)
"""
...
def test_ruff_check_at_or_below_baseline() -> None:
    """ruff lint finding count must not exceed the M1 baseline of 732 lines.

    Pass condition (VAL-M15-M1-LINT-RUFF-CHECK-001):
...
def test_ruff_format_at_or_below_baseline() -> None:
    """ruff format-check file count must not exceed the M1 baseline of 433 files.

    Pass condition (VAL-M15-M1-LINT-RUFF-FORMAT-001):
