...
4. **Do not stage:** Cursor editor-only files under `.cursor/` (for example local settings.json), `desktop/live-smoke/`, repomix outputs, build artifacts, temp scripts, unrelated dirty files.
...
- `frontend/src/features/hermes-workspace/screens/skills/WorkspaceSkillsScreen.tsx` — **Skills** workspace UI (`/workspace/skills`; `/skills` and `/hermes-skills` redirect there); distinct from Cursor operator skills; catalog/install APIs remain under `/api/hermes-skills/*` and `/api/workspace/skills/*`
